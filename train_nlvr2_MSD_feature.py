...
from data import (TokenBucketSampler, DetectFeatLmdb, TxtTokLmdb,
                  ...
                  PrefetchLoader)
from model.nlvr2_msd_feature import (UniterForNlvr2Paired, UniterForNlvr2Triplet,
...
def main(opts):
    ...
    LOGGER.info(f"***** Running training NLVR2 with MSD *****")
            ...
            if (step + 1) % opts.gradient_accumulation_steps == 0:
                ...
                if global_step % opts.valid_steps == 0:
                    for split, loader in [('val', val_dataloader),
                        ...
                        log, results = validate(model, loader, split,eval_threshold =True)
...
@torch.no_grad()
def validate(model, val_loader, split,eval_threshold=True):
    model.eval()
    val_loss = 0
    tot_score = 0
    n_ex = 0
    ...
    for i, batch in enumerate(val_loader):
        qids = batch['qids']
        # print("batch['targets']",batch['targets'].size())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default='paired', choices=['paired', 'triplet', 'paired-attn'],help="choose from 2 model architecture")
    parser.add_argument('--use_img_type', action='store_true', help="expand the type embedding for 2 image types")
