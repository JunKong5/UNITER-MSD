...
class BertEElayer (nn.Module):
    ...
    def forward(self, encoder_outputs):
        ...
        return logits, pooled_output, encoder_outputs[0]
...
class UniterEncoder(nn.Module):
    ...
    def forward(self, input_, attention_mask,
        ...
        for i,layer_module in enumerate( self.layer):
            ...
            EElayer_exit = self.EElayer[i](current_outputs)  # logits  [bs,clasess], pooled_output [ba,hiden],hidden_feature[bs,length,hidden]
            ...
            if not self.training:
                ...
                EElayer_exit = EElayer_exit + (EElayer_entropy,)  # logits, pooled_output, hidden_feature, entropy
                all_EElayer_exits = all_EElayer_exits + (EElayer_exit,)


                if EElayer_entropy < self.early_exit_entropy[i]:

                    new_output = (EElayer_logits,) + (EElayer_feature,) + (hidden_feature,) + current_outputs[1:] + (all_EElayer_exits ,) # EElogits, EEfeature, all_hidden_states, all_attentions, all_EElayer_exits
